...
DOCKER_TAG=$1
DOCKER_IMAGE=$AWS_ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/aws101:$DOCKER_TAG
ECS_SERVICE_NAME=aws101-todo-app
ECS_CLUSTER_NAME=$(aws ecs list-clusters --output text --query 'clusterArns[0]')
ECS_EXECUTION_ROLE=$(aws iam get-role --role-name aws101-application-network-ECSTaskExecutionRole-1DR8RAZ6ZKYQJ --output text --query 'Role.Arn')

cat ecs_task_template.json | jq --arg img "$DOCKER_IMAGE" --arg role "$ECS_EXECUTION_ROLE" '.containerDefinitions[0].image = $img | .executionRoleArn = $role' > revision.json
