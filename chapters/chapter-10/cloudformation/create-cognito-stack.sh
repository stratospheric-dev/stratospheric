...
USER_POOL_DOMAIN=$(aws cloudformation describe-stacks --stack-name stratospheric-cognito-test --output text --query 'Stacks[0].Outputs[?OutputKey==`UserPoolDomain`].OutputValue')

echo $USER_POOL_ID
echo $USER_POOL_PROVIDER_URL
echo $USER_POOL_CLIENT_ID
echo $USER_POOL_CLIENT_SECRET
echo $USER_POOL_DOMAIN
